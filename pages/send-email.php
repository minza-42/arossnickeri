...
if (!empty($errors)) {
    header("Location: kontakt.html?error=" . urlencode(implode(", ", $errors)));
    exit;
}
...
if ($mail_sent) {
    ...
    header("Location: tack.html?namn=" . urlencode($namn));
    exit;
} else {
    // Om e-post inte kunde skickas
    header("Location: kontakt.html?error=" . urlencode("Ett fel uppstod. Försök igen eller kontakta oss direkt via telefon."));
    exit;
}
